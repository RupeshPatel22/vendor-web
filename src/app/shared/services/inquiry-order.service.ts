...
export class InquiryOrderService {
  ...
  removeSelectedItemDetails(item: any): void {
    // Implement the logic to remove the item from addNewSelectedItemsDetails
    // For example, if addNewSelectedItemsDetails is an array, you can do something like:
    const index = this.addNewSelectedItemsDetails.findIndex(selectedItem => selectedItem.itemId === item.menu_item_id);
    console.log(this.addNewSelectedItemsDetails);
    console.log(item,'item')
    if (index !== -1) {
        console.log(`Removing item from addNewSelectedItemsDetails at index ${index}`);
