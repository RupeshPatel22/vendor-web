...
export class InquiryOrderDetailsDialogComponent implements OnInit, OnDestroy {
  ...
  rejectOrder(): void {
    ...
    this.inquiryOrderService.putInquiryOrdersDetails(this.inquiryOrderDetails.inquiryOrderId, data).subscribe(res => {
    this.toastMsgService.showSuccess(`Order Id: ${this.inquiryOrderDetails.inquiryOrderId} is rejected successfully`);
   ...
   * @returns 
   */
  editQuantityOfMenuItems(inquiryMenuItemId: number, menuItemId: number, menuItemName: string, quant:number): void {
    if (this.isOutOfStockForItem(inquiryMenuItemId)) {  
      this.toastMsgService.showError("This item is marked out of stock. Please mark it as in stock before editing qunatity.");
      return;
    }

    for (const i of this.selectAlt){
      if(i.inquiry_item_id === inquiryMenuItemId){
        if(!i.replacements.length){
          this.selectAlt.splice(i, 1);
        }
        else{
      this.toastMsgService.showError("Please remove the alternative selection before editing the quantity.");
      return;
        }
      }
    }
  ...
  selectAlternativeMenuItems(masterCategoryId: number, inquiryItemId: number, menuItemId: number): void {
    if (this.isOutOfStockForItem(inquiryItemId)) {
      this.toastMsgService.showError("This item is marked out of stock. Please mark it as in stock before selecting alternatives.");
      return;
    }
    for (const i of this.editQuantity){
      if(i.inquiry_item_id === inquiryItemId){
        if(!i.replacements.length){
          this.editQuantity.splice(i, 1);  
              }
        else{
      this.toastMsgService.showError("Please remove edited quantity before selecting alternative.");
      return;
        }
      }
    }
  ...
  markOutOfStockMenuItems(inquiryMenuItemId: number): void {
    ...
    if (existingEditIndex !== -1 || existingAltIndex !== -1) {
  ...
  removeFromArray(array: any[], inquiryMenuItemId: number, type?: any): void {
      ...
      if (item.menu_item_id === inquiryMenuItemId) {
        ...
        if (type === "replacement") {
          ...
          if (replacementIndex !== -1) {
            item.replacements.splice(replacementIndex, 1);
    ...
    if (index !== -1 && type !== "replacement") {
      array.splice(index, 1); // Remove the main item only if it's not a replacement removal
    }
  }
